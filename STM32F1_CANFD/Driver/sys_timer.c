...
void send_canfd_k2_test()//传统CAN
{
	...
	can_tx_msg.head.bF.ctrl.DLC = CAN_DLC_8;	//长度参考“CAN_DLC”定义，不可以填定义里没有的参数
	can_tx_msg.head.bF.ctrl.IDE = 1;	// Extended CAN ID false
	can_tx_msg.head.bF.ctrl.RTR = 0;	// Remote frame
	can_tx_msg.head.bF.ctrl.BRS = FALSE;	//切换速率，也就是数据域启动高速率发送/接收
...
void send_canfd_k3_test()
{
	...
	can_tx_msg.head.bF.ctrl.IDE = 1;	// Extended CAN ID false
	can_tx_msg.head.bF.ctrl.RTR = 0;	// Remote frame
	can_tx_msg.head.bF.ctrl.BRS = TRUE;	//BRS(Bit Rate Switch)位速率转换开关，当BRS为显性位时数据段的位速率与仲裁段的位速率一致，当BRS为隐性位时数据段的位速率高于仲裁段的位速率
	can_tx_msg.head.bF.ctrl.FDF = TRUE;	// CAN FD
	for(int j=0;j<8;j++)
	{
		can_tx_msg.dat[j] = j+1;
