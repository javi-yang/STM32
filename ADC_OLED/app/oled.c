...
void OLED_ShowCHinese(uint8_t x, uint8_t y, uint8_t no)
{
    ...
    for(i = 0; i < 16; i++)
    {
        OLED_WR_Byte(rebo[no << 1][i], OLED_DATA);
    }
    ...
    for(i = 0; i < 16; i++)
    {
        OLED_WR_Byte(rebo[(no << 1) + 1][i], OLED_DATA);
    }
}

void OLED_ShowDateNum_x12(uint8_t x, uint8_t y, uint8_t no)
{
    unsigned char i = 0;
    OLED_Set_Pos(x, y);
    ...
    for(i = 0; i < 16; i++)
    {
        OLED_WR_Byte(DATE_FONT[no << 1][i], OLED_DATA);
    }
    ...
    for(i = 0; i < 16; i++)
    {
        OLED_WR_Byte(DATE_FONT[(no << 1) + 1][i], OLED_DATA);
    }
}
